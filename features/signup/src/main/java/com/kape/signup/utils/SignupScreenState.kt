...
val ERROR_EMAIL_INVALID =
    SignupScreenState(
        loading = false,
        SignupStep.Default,
        error = SignupError.EmailInvalid,
    )
val ERROR_REGISTRATION =
    SignupScreenState(
        loading = false,
        SignupStep.Default,
...
fun SUBSCRIPTIONS(data: SubscriptionData) = SignupScreenState(loading = false, SignupStep.Subscriptions(true), subscriptionData = data)

val SUBSCRIPTIONS_FAILED_TO_LOAD =
    SignupScreenState(
        loading = false,
...
fun signedUp(credentials: Credentials) = SignupScreenState(loading = false, SignupStep.SignedUp(credentials))
...
data class SubscriptionData(
    val selected: MutableState<Plan>,
    val yearly: Plan,
    val monthly: Plan,
)
